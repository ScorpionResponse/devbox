...
Vagrant.configure("2") do |config|
  ...
  # Avoid the 'stdin: is not a tty' error
  ...
  # THIS NEXT PART IS TOTAL HACK (only necessary for running Vagrant on
  # Windows) Windows currently doesn't support SSH Forwarding when running
  # Vagrant's "Provisioning scripts" (e.g. all the "config.vm.provision"
  # commands below). Although running "vagrant ssh" (from Windows commandline)
  # will work for SSH Forwarding once the VM has started up,
  # "config.vm.provision" commands in this Vagrantfile DO NOT.  Supposedly
  # there's a bug in 'net-ssh' gem (used by Vagrant) which causes SSH
  # forwarding to fail on Windows only
  # See: https://github.com/mitchellh/vagrant/issues/1735
  #      https://github.com/mitchellh/vagrant/issues/1404
  # See also underlying 'net-ssh' bug:
  # https://github.com/net-ssh/net-ssh/issues/55
  #
  # Therefore, we have to "hack it" and manually sync our SSH keys to the
  # Vagrant VM & copy them over to the 'root' user account (as 'root' is the
  # account that runs all Vagrant "config.vm.provision" scripts below). This
  # all means 'root' should be able to connect to GitHub as YOU! Once this
  # Windows bug is fixed, we should be able to just remove these lines and
  # everything should work via the "config.ssh.forward_agent=true" setting.
  # ONLY do this hack/workaround if the local OS is Windows.
  if Vagrant::Util::Platform.windows?
    # MORE SECURE HACK. You MUST have a ~/.ssh/github_rsa (GitHub specific) SSH
      # key to copy to VM (ensures we are not just copying all your local SSH
      # keys to a VM)
    if File.exists?(File.join(Dir.home, ".ssh", "github_rsa"))
      ...
      # Else, throw a Vagrant Error. Cannot successfully startup on Windows
  ...
  # Create a '/etc/sudoers.d/root_ssh_agent' file which ensures sudo keeps any
  # SSH_AUTH_SOCK settings This allows sudo commands (like "sudo ssh
  # git@github.com") to have access to local SSH keys (via SSH Forwarding) See:
  ...
  # Turn off annoying console bells/beeps in Ubuntu (only if not already turned
  # off in /etc/inputrc)
